	...
	try {
		...
		if (isGroupMsg && GroupLinkDetector && !isGroupAdmins && !isOwner) {
			if (chats.match(/(https:\/\/chat.whatsapp.com)/gi)) {
				...
				if (chats.includes(inviteLink1)) return urbae.reply(from, `This is the group link *${name}* You will not be kicked`, id)
				const check = await urbae.inviteInfo(chats);
				if (!check) {
					return
				} else {
					urbae.reply(from, '*[GROUP LINK DETECTOR]*\nYou sent the group chat link, sorry you were kicked from the group :(', id).then(() => {
		...
		if (isCmd && isMuted(chatId) && banChat() && !isBlocked && !isBanned || isOwnerB || isPrem) {
			switch (command) {
				// Menu and TnC
				case prefix + 'exif':
					if (!isOwnerB) return urbae.reply(from, 'This command can only be used by Owner Bot!', id)
					...
					packstc = packauthor
					urbae.reply(from, `Successfully changed the sticker pack to ${splitauthor} and ${packauthor}`, id)
					break
				case prefix + 'mute':
					if (!isGroupMsg) return urbae.reply(from, 'This feature can only be used in Groups!', id)
					if (!isOwnerB) return urbae.reply(from, 'This feature can only be used by Owner Bot!', id)
					if (isGroupMsg) {
						isMuted(chatId) == true
						if (muted.includes(chatId)) return urbae.reply(from, 'This group has been muted before', id)
						muted.push(chatId)
						fs.writeFileSync('./lib/database/muted.json', JSON.stringify(muted, null, 2))
						urbae.reply(from, 'The bot has been muted in this group!', id)
					} else {
						muted.push(chatId)
						fs.writeFileSync('./lib/database/muted.json', JSON.stringify(muted, null, 2))
						urbae.reply(from, 'The bot has been muted in this group!', id)
				...
				case prefix + 'unmute':
					...
					if (isGroupMsg) {
						...
						fs.writeFileSync('./lib/database/muted.json', JSON.stringify(muted, null, 2))
						urbae.reply(from, 'The bot has been unmuted in this group!', id)
					} else {
						let indexsz = muted.indexOf(chatId);
						muted.splice(indexsz, 1)
						fs.writeFileSync('./lib/database/muted.json', JSON.stringify(muted, null, 2))
					...
					break
				case prefix + 'private':
					if (!isOwnerB) return urbae.reply(from, 'This command can only be used by the Bot owner!', id)
				...
				case prefix + 'setpic':
					...
					if (isMedia) {
						...
						urbae.sendTextWithMentions(from, `Thank you @${serial} Profile photo..`)
					} else if (quotedMsg && quotedMsg.type == 'image') {
						const mediaData = await decryptMedia(quotedMsg)
						const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
						await urbae.setProfilePic(imageBase64)
						urbae.sendTextWithMentions(from, `Thank you @${serial} Profile photo`, id)
					} else {
						urbae.reply(from, `Wrong Format!\n⚠️ Please Send Image With ${prefix}setpic`, id)
					}
					break
				case prefix + 'getpic':
					if (!isGroupMsg) return urbae.reply(from, `This feature can only be used in groups`, id)
					...
					try {
						...
						urbae.reply(from, `No Profile Photo!`, id)
					}
					break
				case prefix + 'tnc':
					...
					break
				case prefix + 'help':
					const bots = `Hi ${pushname}, this is Sam Bot, to find out the commands menu, type *${prefix}menu* , *${prefix}p*`
					...
					axios.get(`http://api.lolhuman.xyz/api/ceritahoror?apikey=${lolhuman}`)
						.then(async (res) => {
							...
							await urbae.sendFileFromUrl(from, res.data.result.thumbnail, 'thumb.jpg', becapt, id)
								.catch(() => {
									urbae.reply(from, 'The server is under maintenance or the apikey has a limit', id)
								})
						})
						.catch(err => {
							console.log(err)
				...
				case prefix + 'eval':
				case prefix + 'ev':
					if (!isOwner) return await urbae.reply(from, 'This command can only be used by the Owner!', id)
					if (!q) return await urbae.reply(from, 'Enter the Javascript code!', id)
					try {
						let evaled = await eval(q)
						if (typeof evaled !== 'string') evaled = require('util').inspect(evaled)
					} catch (err) {
						console.error(err)
						urbae.reply(from, 'Js code is wrong', id)
					}
					break
				case prefix + 'p':
				case prefix + 'start':
				...
				case prefix + 'menuadmin':
					if (!isGroupMsg) return urbae.reply(from, 'Sorry, this command can only be used in groups!', id)
					if (!isGroupAdmins) return urbae.reply(from, 'Failed, remember you are a member not an admin', id)
					const php4 = menupict
				...
				case prefix + 'donasi':
					...
					break
				case prefix + 'tod':
					urbae.reply(from, `Before playing, promise to carry out whatever orders are given.\n\nPlease select:\n➥ ${prefix}truth\n➥ ${prefix}dare`, id)
					break
				case prefix + 'rneko':
					urbae.reply(from, mess.wait, id)
					...
					break
				case prefix + 'truth':
					if (!isGroupMsg) return urbae.reply(from, 'This command can only be used in groups!', id)
					fetch('https://raw.githubusercontent.com/AlvioAdjiJanuar/random/main/truth.txt')
						...
						.catch(() => {
							urbae.reply(from, 'Hey, there is an error!!', id)
						})
					break
				case prefix + 'dare':
					if (!isGroupMsg) return urbae.reply(from, 'This command can only be used in groups!', id)
				...
				case prefix + 'citacita'://Piyobot
					...
					break
				case prefix + 'kbbi':
					if (args.length == 0) return urbae.reply(from, `To search for a word from the Big Indonesian Dictionary (KBBI)\ntype: ${prefix}kbbi [kata]`, id)
					...
					break
				case prefix + 'marvel':
					if (args.length == 0) return urbae.reply(from, `Send command ${prefix}marvel text1text2\nExample: ${prefix}marvel Sam Bot`, id)
					urbae.reply(from, mess.wait, id)
				...
				case prefix + 'blackpink':
					if (args.length == 0) return urbae.reply(from, `send command ${prefix}blackpink name`, id)
					urbae.reply(from, mess.wait, id)
				...
				case prefix + 'glowtext':
					if (args.length == 0) return urbae.reply(from, `send command ${prefix}logoff [name]`, id)
					urbae.reply(from, mess.wait, id)
				...
				case prefix + 'logoff':
					...
					break
				case prefix + 'logoph':
					if (args.length === 0) return urbae.reply(from, `Send command *${prefix}logoph Text1 Text2 ]*,\n\n example : *${prefix}logoph Sam Bot*`, id)
					const lpornhub = args[0]
					const lpornhub2 = args[1]
					if (lpornhub > 10) return urbae.reply(from, '*Text1 Too Long!*\n_Maximum 10 letters!_', id)
					if (lpornhub2 > 10) return urbae.reply(from, '*Text2 Too Long!*\n_Maximum 10 letters!_', id)
				...
				case prefix + 'level':
					if (!isLevelingOn) return await urbae.reply(from, 'The leveling feature is not yet active in this group!', id)
					if (!isGroupMsg) return await urbae.reply(from, 'This feature can only be used in Groups!', id)
					const userLevel = level.getLevelingLevel(sender.id, _level)
				...
				case prefix + 'leveling':
					if (!isGroupMsg) return await urbae.reply(from, 'This feature can only be used in Groups!', id)
					if (!isGroupAdmins) return await urbae.reply(from, 'This feature can only be used by Group Admins!', id)
					if (args[0] === 'on') {
						if (_leveling.includes(groupId)) return urbae.reply(from, 'The leveling feature has been activated in this group before', id)
						_leveling.push(groupId)
						fs.writeFileSync('./lib/database/group/leveling.json', JSON.stringify(_leveling))
						await urbae.reply(from, 'The Leveling feature has been successfully activated in this group', id)
					} else if (args[0] === 'off') {
						var thisgroup = _leveling.indexOf(groupId)
						_leveling.splice(thisgroup, 1)
						fs.writeFileSync('./lib/database/group/leveling.json', JSON.stringify(_leveling))
						await urbae.reply(from, 'Leveling feature has been successfully disabled in this group', id)
					} else {
						await urbae.reply(from, 'Choose on or off cute admin', id)
					}
					break
				case prefix + 'leaderboard':
					...
					try {
						...
						console.error(err)
						await urbae.reply(from, 'Requires at least *10* users who have a level in the database!', id)
					}
					break
				case prefix + 'slightning':
					if (isMedia && type === 'image') {
						...
						await urbae.reply(from, `Wrong Format!\n⚠️ Please Send Image With #stickerlightning`, id)
					}
					break
				case prefix + 'sfire':
				case prefix + 'stickerfire':
					if (isMedia && type === 'image') {
						...
						await urbae.reply(from, `Wrong Format!\n⚠️ Please Send Image With ${prefix}stickerfire`, id)
					}
					break
				case prefix + 'thunder':
					if (args.length === 1) return urbae.reply(from, `Send command *${prefix}thunder [ Text ]*, eg *${prefix}thunder Tobz*`, id)
					urbae.reply(from, mess.wait, id)
					const thndr = body.slice(9)
					if (thndr.length > 10) return urbae.reply(from, '*Text Too Long!*\n_Maximum 10 letters!_', id)
					await urbae.sendFileFromUrl(from, `https://api.vhtear.com/thundertext?text=${thndr}&apikey=${vhtearkey}`, 'thndr.jpg', '', id)
					break

				case prefix + 'guessthepic':
					if (!isGroupMsg) return urbae.reply(from, 'This command can only be used in groups!', id)
					try {
						...
						urbae.sendFileFromUrl(from, resp.data.result.images, 'guessthepic.jpg', '_Please answer the meaning of this picture_', id)
						urbae.sendText(from, `30 Seconds left...`, id)
						await sleep(10000)
						urbae.sendText(from, `20 Seconds left...`, id)
						await sleep(10000)
						urbae.sendText(from, `10 Seconds left...`, id)
						await sleep(10000)
						urbae.reply(from, jwban, id)
					} catch (err) {
						console.error(err.message)
						await urbae.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Sorry, Quiz Question not found')
					}
					break
				case prefix + 'ricecake':
					if (!isGroupMsg) return urbae.reply(from, 'This command can only be used in groups!', id)
					try {
				...
				case prefix + 'maps':
				case prefix + 'map':
					if (args.length == 0) return urbae.reply(from, `Search for a city from google map\nUsage: ${prefix}maps namakota\nExample: ${prefix}maps pontianak`, id)
					urbae.reply(from, mess.wait, id)
					await urbae.sendFileFromUrl(from, `https://api.clph.me/map?apikey=${caliph}&kota=${body.slice(6)}`)
						.catch(() => {
							urbae.reply(from, 'Rest Api is in error', id)
						})
					break
				...
				case prefix + 'autostik':
					if (args.length == 0) return urbae.reply(from, `Select enable or disable`, id)
					if (args[0] === 'enable') {
						if (isAutoStikerOn) return await urbae.reply(from, 'The auto sticker feature has been activated', id)
						_autostiker.push(chat.id)
						fs.writeFileSync('./lib/helper/autosticker.json', JSON.stringify(_autostiker))
						await urbae.reply(from, 'The autosticker feature has been activated successfully', id)
					} else if (args[0] === 'disable') {
						let ingroup = _autostiker.indexOf(groupId)
						_autostiker.splice(ingroup, 1)
						fs.writeFileSync('./lib/helper/autosticker.json', JSON.stringify(_autostiker))
						await urbae.reply(from, 'The autosticker feature has been disabled successfully', id)
					} else {
						await urbae.reply(from, 'Incorrect format', id)
					}
					break
				case prefix + 'neko':
					try {
				...
				case prefix + 'antilink':
					...
					if (args[0] == 'on') {
						...
						if (cek) {
							return urbae.reply(from, '*Anti Group Link Detector* already active in this group', id) //if number already exists on database
						} else {
							antilink.push(chatId)
							fs.writeFileSync('./lib/helper/antilink.json', JSON.stringify(antilink))
							urbae.reply(from, '*[Anti Group Link]* has been activated\nAny group member who sends a message containing a group link will be kicked by a bot!', id)
						}
					} else if (args[0] == 'off') {
						var cek = antilink.includes(chatId);
						if (!cek) {
							return urbae.reply(from, '*Anti Group Link Detector* already inactive in this group', id) //if number already exists on database
						} else {
							let nixx = antilink.indexOf(chatId)
							antilink.splice(nixx, 1)
							fs.writeFileSync('./lib/helper/antilink.json', JSON.stringify(antilink))
							urbae.reply(from, '*[Anti Group Link]* has been disabled\n', id)
						}
					} else {
						urbae.reply(from, `pilih on / off\n\n*[Anti Group Link]*\nAny group member who sends a message containing the group link will be kicked by a bot!`, id)
					}
					break
				case prefix + 'inv':
				case prefix + 'invite':
				...
				case prefix + 'ava':
					...
					try {
						...
						if (dp == undefined) {
							var pfp = urbae.reply(from, 'He is shy, maybe he is depressed, he does not dare to post a profile picture', id)
						} else {
							var pfp = urbae.sendFileFromUrl(from, dp, 'profile.png')
						}
					} catch {
						urbae.reply(from, 'No profile/private photo', id)
					}
					break
				case prefix + 'mystat': {
					...
					var adm = admins
					if (ct == null) {
						return await urbae.reply(from, 'Invalid WhatsApp number [ Not registered on WhatsApp ]', id)
					} else {
						...
						if (contact == undefined) {
							var nama = 'there is no name, maybe wibu'
						} else {
							var nama = contact
						}
				...
				case prefix + 'jadian':
					...
					const sapa = `Cieee... @${aku.replace(/[@c.us]/g, '')} (💘) @${kamu.replace(/[@c.us]/g, '')} this is new\nShare the pj please`
					await urbae.sendTextWithMentions(from, sapa)
					break
				...
				case prefix + 'bkp': // MFARELS
					...
					break
				case prefix + 'join':
					if (args.length == 0) return urbae.reply(from, `If you want to invite bots to a group, please invite or by typing ${prefix}join [link group]`, id)
					...
					if (!islink) return urbae.reply(from, 'Sorry the group link is wrong! please send the correct link', id)
					if (isPrem) {
						await urbae.joinGroupViaLink(linkgrup)
							.then(async () => {
								await urbae.sendText(from, 'Successfully joined the group via the link!')
								await urbae.sendText(chekgrup.id, `what up y'all , I'm Sam Bot. To find out the commands on this Bot type ${prefix}menu`)
							})
					} else {
						...
						await urbae.joinGroupViaLink(linkgrup)
							.then(async () => {
								await urbae.reply(from, 'Successfully joined the group via the link!', id)
							})
							.catch(() => {
								urbae.reply(from, 'Failed!', id)
							})
					}
					break
				case prefix + 'wattpadstory':
					if (args.length == 0) return urbae.reply(from, `To search stories from wattpad! use ${prefix}wattpadstory url story\nContoh : ${prefix}wattpadstory https://www.wattpad.com/story/226120582-my-teacher-levi-x-student-reader`, id)
				...
				case prefix + 'youwatch':
					if (args.length == 0) return urbae.reply(from, `Searching for a drama film from the You Watch website!\nExample : ${prefix}youwatch vagabond`, id)
					...
					break
				case prefix + 'shopee':
					if (args.length === 0) return urbae.reply(from, `Send orders *${prefix}shopee [ Query ]*, Contoh : *${prefix}shopee HP Samsul a20*`, id)
					...
					break
				case prefix + 'weather':
					if (args.length == 0) return urbae.reply(from, `To see the weather for an area\ntype: ${prefix}cuaca district/city name of city/district\nExample of district: ${prefix}cuaca kab Bengkayang\nContoh Kota: ${prefix}cuaca kota Pontianak`, id)
					...
					try {
						...
						urbae.reply(from, 'Sorry, the area you are looking for is not available', id)
						console.log(err)
					}
					break
				...
				case prefix + 'playstore':
					if (args.length === 0) return urbae.reply(from, `Send orders *${prefix}playstore [ Query ]*, Example : *${prefix}playstore Mobile Legends*`)
					...
					try {
						...
						if (dataplay.status == false) return urbae.reply(from, 'The application you are looking for cannot be found or maybe Rest Api is in error', id)
				...
				case prefix + 'infoapp':
				case prefix + 'infoapk':
					if (args.length == 0) return urbae.reply(from, `Enter application id\nExample: ${prefix}infoapp com.tencent.ig`, id)
					urbae.reply(from, mess.wait, id)
					...
					axios.get(`https://dapuhy-api.herokuapp.com/api/search/playstore-info?id=${idaplikasi}&apikey=${dapuhyapi}`)
						.then(async (res) => {
							if (res.data.status == false) return urbae.reply(from, 'The apk id you are looking for cannot be found or maybe Rest Api is in error', id)
				...
				case prefix + 'reportbug':
					...
					await urbae.sendFileFromUrl(ownerNumber, profilepicc, 'prof.jpg', `Bug report from: *${pushname}*\nNumber : ${serial.replace('@c.us', '')}\n\nBug : *${reporter}*`, id)
				...
				case prefix + 'setgroupname':
					...
					urbae.sendTextWithMentions(from, `The group name has been changed by admin @${sender.id.replace('@c.us', '')}\n\n• Before: ${sebelum}\n• After: ${namagrup}`)
					break
				case prefix + 'setname':
					if (!isOwnerB) return urbae.reply(from, `This command can only be used by Owner Bot!`, id)
					const setnem = body.slice(9)
					await urbae.setMyName(setnem)
					urbae.sendTextWithMentions(from, `Thanks for the new name @${sender.id.replace('@c.us', '')} 😘`)
					break
				case prefix + 'read':
					if (!isGroupMsg) return urbae.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
					if (!quotedMsg) return urbae.reply(from, `Please Reply Bot Message`, id)
					if (!quotedMsgObj.fromMe) return urbae.reply(from, `Please Reply Bot Message`, id)
					try {
						...
						urbae.sendTextWithMentions(from, `Just reading.. Come on, bro\n${list}`)
					} catch (err) {
						console.log(err)
						urbae.reply(from, `Sorry, Nobody Reads Bot Messages Yet Or They Disabled Read Receipts`, id)
					}
					break
				case prefix + 'setstatus':
					...
					urbae.sendTextWithMentions(from, `Thanks for the new status @${sender.id.replace('@c.us', '')} 😘`)
					break
				case prefix + 'botstat': {
					const loadedMsg = await urbae.getAmountOfLoadedMessages()
					const charged = await urbae.getIsPlugged();
				...
				case prefix + 'whatsong':
					if (quotedMsg && quotedMsg.type == 'audio' || quotedMsg && quotedMsg.type == 'ptt' || quotedMsg && quotedMsg.type == 'video') {
						try {
							...
							urbae.reply(from, `*Title:* ${detmus.data.title}\n*Artis:* ${detmus.data.artists}\n*Album:* ${detmus.data.album}\n*Release Date:* ${detmus.data.release_date}`, id)
						} catch (err) {
							console.log(err)
							urbae.reply(from, 'Sorry, the song could not be found', id)
						}
				...
				case prefix + 'curi':
					if (quotedMsg && quotedMsg.type == 'sticker') {
						...
						urbae.reply(from, 'Reply the sticker bro', id)
					}
					break
				case prefix + 'take':
				case prefix + 'takestick':
					if (quotedMsg && quotedMsg.type == 'sticker' || quotedMsg && quotedMsg.type == 'image') {
						if (!q.includes('|')) return await urbae.reply(from, `To change the watermark sticker, reply to the sticker/image with the caption ${prefix}take package_name | author_name\n\nExample: ${prefix}takestick PUNYA GUA | videfikri`, id)
						await urbae.reply(from, mess.wait, id)
						...
						urbae.reply(from, 'wrong message format', id)
					}
					break
				...
				case prefix + 'sgiffull':
					if (isMedia && type === 'video' || mimetype == 'sticker/gif') {
						...
						try {
							...
							console.log(err)
							urbae.reply(from, 'The length of the video is too long, please reduce it a bit\nminimum 9 seconds', id)
						}
